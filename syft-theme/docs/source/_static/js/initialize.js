...
function buildHeader() {
  ...
  logo.setAttribute('id', 'openmined-logo');
  logo.src = getLogoSrc(isDarkMode);
  logo.alt = "Openmined logo";
  homeLink.appendChild(logo);

  // logo.parentElement.classList.add('logo-link');

  const childElement = document.getElementById('openmined-logo');
  if (childElement) {
    childElement.parentElement.classList.add('logo-link');
  }
